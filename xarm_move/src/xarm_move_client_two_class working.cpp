...
public:
    explicit XarmMoveClientNode(const rclcpp::NodeOptions& options, std::shared_ptr<geometry_msgs::msg::PointStamped> target, std::mutex& target_mutex) : Node("xarm_move_client_node", options),
    ...
    int execute(std::shared_ptr<geometry_msgs::msg::PointStamped> target_point)
        {
            ...
            {
                std::lock_guard<std::mutex> lock(this->target_mutex);
...
int main(int argc, char** argv)
{
    ...
    std::shared_ptr<geometry_msgs::msg::PointStamped> target = std::make_shared<geometry_msgs::msg::PointStamped>();
    std::mutex target_mutex;
    auto node = std::make_shared<XarmMoveClientNode>(node_options, target, target_mutex);
        ...
        while (rclcpp::ok()) {
            {
                ...
                *target = service_node->getTarget();
                RCLCPP_INFO(service_node->get_logger(), "target at %f, %f, %f", target->point.x, target->point.y, target->point.z);
            }
            sleep(10.0);
        }
    });
